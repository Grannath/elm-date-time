            ...
            [ fuzz (intRange -400 2020) "is correct given any year" <|
                \year ->
                    if List.member year validLeapYears then
                        Expect.true ("Expected " ++ toString year ++ " to be a leap year") (isLeapYear (date year 1 1))
                    else
                        Expect.false (toString year ++ " is not a leap year") (isLeapYear (date year 1 1))
            ]
